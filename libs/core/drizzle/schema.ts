...
export const userReviewMovieLikes = pgTable(
  ...
  (table) => [
    index('idx_user_review_movie_likes_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_review_movie_likes_review_id').using(
      'btree',
      table.reviewId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_review_movie_likes_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
      columns: [table.reviewId],
      foreignColumns: [userReviewsMovie.id],
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('User can insert own like if is allowed to see review', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
  ...
  WHERE (user_reviews_movie.id = user_review_movie_likes.review_id))))`,
    ...
    pgPolicy('User can delete his own like', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
    pgPolicy('Enable read access', {
      as: 'permissive',
      for: 'select',
      to: ['public'],
    }),
...
export const userReviewTvSeriesLikes = pgTable(
  ...
  (table) => [
    index('idx_user_review_tv_series_likes_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_review_tv_series_likes_review_id').using(
      'btree',
      table.reviewId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_review_tv_series_likes_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
      columns: [table.reviewId],
      foreignColumns: [userReviewsTvSeries.id],
...
export const playlistItemsMovie = pgTable(
  ...
  (table) => [
    index('idx_playlist_items_movie_movie_id').using(
      'btree',
      table.movieId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlist_items_movie_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    index('playlist_items_movie_playlist_id').using(
      'btree',
      table.playlistId.asc().nullsLast().op('int8_ops'),
    ),
    index('playlist_items_movie_rank').using(
      'btree',
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('Insert for authorized users', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
      withCheck: sql`(utils_check_playlist_type(playlist_id, 'movie'::text) AND ((utils_check_playlist_owner(( SELECT auth.uid() AS uid), playlist_id) OR utils_check_playlist_guest(( SELECT auth.uid() AS uid), playlist_id)) AND (( SELECT auth.uid() AS uid) = user_id)))`,
    }),
    pgPolicy('Delete for authorized users', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
    pgPolicy('Select for public or authorized users', {
      as: 'permissive',
      for: 'select',
      to: ['public'],
    }),
    pgPolicy('Update for authorized users', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
    check('playlist_items_movie_comment_check', sql`length(comment) <= 180`),
    check('playlist_items_movie_rank_check', sql`rank >= 1`),
  ],
);

export const playlistItemsTvSeries = pgTable(
  'playlist_items_tv_series',
  {
    ...
    playlistId: bigint('playlist_id', { mode: 'number' }).notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    tvSeriesId: bigint('tv_series_id', { mode: 'number' }).notNull(),
    userId: uuid('user_id').notNull(),
    comment: text(),
    rank: integer().notNull(),
  },
  (table) => [
    index('idx_playlist_items_tv_series_tv_series_id').using(
      'btree',
      table.tvSeriesId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlist_items_tv_series_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    index('playlist_items_tv_series_playlist_id').using(
      'btree',
      table.playlistId.asc().nullsLast().op('int8_ops'),
    ),
    index('playlist_items_tv_series_rank').using(
      'btree',
      table.rank.asc().nullsLast().op('int4_ops'),
    ),
    foreignKey({
      columns: [table.tvSeriesId],
    ...
    pgPolicy('Delete for authorized users', {
      ...
      for: 'delete',
      to: ['authenticated'],
      using: sql`(utils_check_playlist_owner(( SELECT auth.uid() AS uid), playlist_id) OR utils_check_playlist_guest(( SELECT auth.uid() AS uid), playlist_id))`,
    }),
    pgPolicy('Insert for authorized users', {
      as: 'permissive',
      for: 'insert',
    ...
    check(
      ...
      sql`length(comment) <= 180`,
    ),
    check('playlist_items_tv_series_rank_check', sql`rank >= 1`),
  ],
);
...
export const userReviewsMovie = pgTable(
  'user_reviews_movie',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).generatedByDefaultAsIdentity({
  ...
  (table) => [
    index('idx_user_reviews_movie_comments_count').using(
      'btree',
      table.commentsCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_reviews_movie_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_reviews_movie_likes_count').using(
      'btree',
      table.likesCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_reviews_movie_updated_at').using(
      'btree',
      table.updatedAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_reviews_movie_views_count').using(
      'btree',
      table.viewsCount.asc().nullsLast().op('int8_ops'),
    ),
    foreignKey({
      columns: [table.id],
      foreignColumns: [userActivitiesMovie.id],
  ...
  WHERE ((uma.id = user_reviews_movie.id) AND (uma.user_id = ( SELECT auth.uid() AS uid)))))`,
    ...
    check('user_reviews_movie_body_check', sql`length(body) <= 50000`),
    check(
      'user_reviews_movie_body_html_wrapper_check',
      sql`body ~ '^<html>.*</html>$'::text`,
    ),
    check('user_reviews_movie_comments_count_check', sql`comments_count >= 0`),
    check('user_reviews_movie_likes_count_check', sql`likes_count >= 0`),
    check('user_reviews_movie_title_check', sql`length(title) <= 50`),
    check('user_reviews_movie_views_count_check', sql`views_count >= 0`),
  ],
);

export const userReviewsTvSeries = pgTable(
  ...
  (table) => [
    index('idx_user_reviews_tv_series_comments_count').using(
      'btree',
      table.commentsCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_reviews_tv_series_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_reviews_tv_series_likes_count').using(
      'btree',
      table.likesCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_reviews_tv_series_updated_at').using(
      'btree',
      table.updatedAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_reviews_tv_series_views_count').using(
      'btree',
      table.viewsCount.asc().nullsLast().op('int8_ops'),
    ),
    foreignKey({
      columns: [table.id],
      foreignColumns: [userActivitiesTvSeries.id],
  ...
  WHERE (uma.id = user_reviews_tv_series.id)))`,
    ...
    check('user_reviews_tv_series_body_check', sql`length(body) <= 50000`),
    check(
    ...
    check(
      ...
      sql`comments_count >= 0`,
    ),
    check('user_reviews_tv_series_likes_count_check', sql`likes_count >= 0`),
    check('user_reviews_tv_series_title_check', sql`length(title) <= 50`),
    check('user_reviews_tv_series_views_count_check', sql`views_count >= 0`),
  ],
);
...
export const playlistsLikes = pgTable(
  'playlists_likes',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).generatedByDefaultAsIdentity({
  ...
  (table) => [
    index('idx_playlists_likes_playlist_id').using(
      'btree',
      table.playlistId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlists_likes_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('User can insert his own like', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
      withCheck: sql`((user_id = ( SELECT auth.uid() AS uid)) AND (EXISTS ( SELECT 1
  ...
  WHERE (playlists.id = playlists_likes.playlist_id))))`,
    ...
    pgPolicy('Like are viewable owner and playlist owner', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
    }),
...
export const playlistGuests = pgTable(
  'playlist_guests',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).generatedByDefaultAsIdentity({
    ...
    playlistId: bigint('playlist_id', { mode: 'number' }).notNull(),
    userId: uuid('user_id').notNull(),
    edit: boolean().default(false).notNull(),
  },
  (table) => [
    index('idx_playlist_guests_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('playlist_guests_playlist_id').using(
      'btree',
      table.playlistId.asc().nullsLast().op('int8_ops'),
    ),
    index('playlist_guests_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('Owner or target guest can delete', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
      using: sql`((( SELECT auth.uid() AS uid) = user_id) OR utils_check_playlist_owner(( SELECT auth.uid() AS uid), playlist_id))`,
    }),
    ...
    pgPolicy('Owner and guests can select', {
      as: 'permissive',
      for: 'select',
      to: ['public'],
    }),
...
export const playlists = pgTable(
  'playlists',
  {
    ...
    likesCount: bigint('likes_count', { mode: 'number' })
      .default(sql`'0'`)
      .notNull(),
    type: playlistsType().notNull(),
  },
  (table) => [
    index('idx_playlists_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_playlists_items_count').using(
      'btree',
      table.itemsCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlists_likes_count').using(
      'btree',
      table.likesCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlists_saved_count').using(
      'btree',
      table.savedCount.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlists_type').using(
      'btree',
      table.type.asc().nullsLast().op('enum_ops'),
    ),
    index('idx_playlists_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('User can insert his own playlist', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
      withCheck: sql`(( SELECT auth.uid() AS uid) = user_id)`,
    }),
    pgPolicy('User can update his own playlist', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
    pgPolicy('User can delete his own playlist', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
    pgPolicy('Playlist are visible depending of the is_public state', {
      as: 'permissive',
      for: 'select',
      to: ['public'],
    }),
    ...
    check('playlists_items_count_check', sql`items_count >= 0`),
    check('playlists_likes_count_check', sql`likes_count >= 0`),
    check('playlists_saved_count_check', sql`saved_count >= 0`),
    check(
...
export const userFollower = pgTable(
  ...
  (table) => [
    index('idx_user_follower_followee_id').using(
      'btree',
      table.followeeId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_user_follower_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    unique('unique_follower_followee').on(table.followeeId, table.userId),
    pgPolicy('Read rule', {
      as: 'permissive',
      for: 'select',
      to: ['public'],
      using: sql`((( SELECT auth.uid() AS uid) = user_id) OR (( SELECT auth.uid() AS uid) = followee_id) OR (((NOT utils_check_user_privacy(followee_id)) OR utils_check_user_following(( SELECT auth.uid() AS uid), followee_id)) AND ((NOT utils_check_user_privacy(user_id)) OR utils_check_user_following(( SELECT auth.uid() AS uid), user_id))))`,
    }),
    ...
    pgPolicy('Followee can update the is_pending column', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
    pgPolicy('Users can delete their own followee or people who follow him', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
    check('check_follower_followee', sql`user_id <> followee_id`),
  ],
);
...
export const userFriend = pgTable(
  ...
  (table) => [
    index('idx_user_friend_friend_id').using(
      'btree',
      table.friendId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_user_friend_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    unique('unique_friend_user').on(table.friendId, table.userId),
    pgPolicy('User can see his friends and users friend with him', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
      using: sql`((( SELECT auth.uid() AS uid) = user_id) OR (( SELECT auth.uid() AS uid) = friend_id))`,
    }),
    check('check_friend_user', sql`user_id <> friend_id`),
  ],
);

export const userRecosMovie = pgTable(
  'user_recos_movie',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).generatedByDefaultAsIdentity({
    ...
    comment: text(),
    status: recoStatus().default('active').notNull(),
  },
  (table) => [
    index('idx_user_recos_movie_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_recos_movie_movie_id').using(
      'btree',
      table.movieId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_recos_movie_sender_id').using(
      'btree',
      table.senderId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_user_recos_movie_status').using(
      'btree',
      table.status.asc().nullsLast().op('enum_ops'),
    ),
    index('idx_user_recos_movie_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    uniqueIndex('user_recos_movie_active_unique')
      .using(
        'btree',
      ...
      .where(sql`(status = 'active'::reco_status)`),
    foreignKey({
      columns: [table.movieId],
      foreignColumns: [tmdbMovie.id],
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('Only friends user can insert.', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
      withCheck: sql`((sender_id = ( SELECT auth.uid() AS uid)) AND (status = 'active'::reco_status) AND utils_check_user_friendship(user_id, ( SELECT auth.uid() AS uid)))`,
    }),
    pgPolicy('Owner or sender can see', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
    }),
    pgPolicy('Sender can update comment only & User can change status', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
    check('user_recos_movie_comment_check', sql`length(comment) <= 180`),
    check(
      'user_recos_movie_unique_user_id_sender_id',
      sql`user_id <> sender_id`,
    ),
  ],
);

export const userRecosTvSeries = pgTable(
  ...
  (table) => [
    index('idx_user_recos_tv_series_created_at').using(
      'btree',
      table.createdAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_user_recos_tv_series_sender_id').using(
      'btree',
      table.senderId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_user_recos_tv_series_status').using(
      'btree',
      table.status.asc().nullsLast().op('enum_ops'),
    ),
    index('idx_user_recos_tv_series_tv_series_id').using(
      'btree',
      table.tvSeriesId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_user_recos_tv_series_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    uniqueIndex('user_recos_tv_series_active_unique')
      .using(
        'btree',
        table.tvSeriesId.asc().nullsLast().op('uuid_ops'),
        table.userId.asc().nullsLast().op('int8_ops'),
        table.senderId.asc().nullsLast().op('uuid_ops'),
      )
      .where(sql`(status = 'active'::reco_status)`),
    ...
    check('user_recos_tv_series_comment_check', sql`length(comment) <= 180`),
    check(
...
export const playlistsSaved = pgTable(
  'playlists_saved',
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: 'number' }).generatedByDefaultAsIdentity({
  ...
  (table) => [
    index('idx_playlists_saved_playlist_id').using(
      'btree',
      table.playlistId.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_playlists_saved_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('User can insert his own saved playlist', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
      withCheck: sql`((user_id = ( SELECT auth.uid() AS uid)) AND (EXISTS ( SELECT 1
  ...
  WHERE (playlists.id = playlists_saved.playlist_id))) AND (NOT utils_check_playlist_owner(( SELECT auth.uid() AS uid), playlist_id)))`,
    ...
    pgPolicy('Saved playlists are viewable owner and playlist owner', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
    }),
    pgPolicy('User can delete his own saved playlist', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
